...
public static class QAction
{
	...
	public static void Run(SLProtocol protocol)
	{
		try
		{
			...
			if (latestQuote == null)
			{
				protocol.Log($"QA{protocol.QActionID}|{protocol.GetTriggerParameter()}|Run| Latest Quote is null", LogType.Error, LogLevel.NoLogging);
				return;
			}
	...
	private static void FillLatestQuoteParameters(SLProtocol protocol, Data latestQuote)
	{
		try
		{
			...
			Dictionary<int, object> parameters = new Dictionary<int, object>
			{
				...
				{ Parameter.latestquotelastupdate_40, latestQuote.LastUpdated.ToLocalTime().ToOADate() },
